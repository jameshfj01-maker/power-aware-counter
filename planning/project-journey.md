...
Tiny Tapeout SKY26c 셔틀 제출을 목표로 설계한 실제 ASIC(주문형 반도체) 프로젝트 (제출 여부는 5절 참고). 8비트 카운터를 매 클럭 증가시키되, 출력을 이진수가 아니라 **그레이코드**로 변환해서 내보낸다. 그레이코드는 값이 바뀔 때 항상 정확히 1비트만 바뀌는 성질이 있어서, 여러 비트가 동시에 스위칭될 때 생기는 전력망 노이즈(di/dt)를 줄인다.
...
**확정적으로 검증된 것:**
- RTL/게이트레벨 시뮬레이션 전부 통과, GDS 빌드까지 통과 (실제 CI 로그로 확인 — 파운드리 제출 가능한 상태까지 완성됐다는 뜻)
- 스위칭 49.8% 감소 (실행해서 확인한 숫자, 디지털 로직 상 항상 성립)
- tt-switching-analyzer 코드 동작 (유닛 테스트 + CI 통과)

**이번엔 하지 않기로 결정한 것:**
- **SKY26c 셔틀 실제 제출은 하지 않는다.** 얼리버드 할인 종료로 풀킷 가격이 370유로로 오른 점, 배송 예상일(2027-05)이 대학 지원 마감(2027-03)보다 늦어 실물 칩이 입시 타임라인에 물리적으로 활용 불가능한 점을 고려한 결정. "1 tile only"(약 12만원) 옵션도 검토했으나, 실물 검증이 불가능한 이상 서사적 차이가 크지 않다고 판단해 제외.
- 그래서 `hardware/hardware_test.py`(SDK 소스코드까지 확인해서 작성)와 오픈소스 도구의 실제 하드웨어 캡처 기능은 **작성은 완료됐지만 실행 검증은 이번엔 하지 않는다.** 졸업 후 본인 비용으로 풀킷을 구매하면 그때 그대로 재사용할 계획.
- 오픈소스 도구(tt-switching-analyzer)의 커뮤니티 반응은 아직 미확인 (공유 예정)

이 구분은 중요하다 — "시뮬레이션으로 검증했다"는 사실이고, "실물로 검증했다"·"커뮤니티에 임팩트를 줬다"는 아직 사실이 아니다. 자소서/면접에서 "실물로 확인해봤냐"는 질문에는: "칩 제작은 보통 반년 이상 걸리기 때문에, 시뮬레이션 단계에서 최대한 엄밀하게 검증했고(GDS 빌드까지 통과), 그 검증 로직 자체를 오픈소스 도구로 공개했다"고 답하는 게 정직하면서도 산업 현실을 이해하고 있다는 인상을 준다.
...
- (보류, 졸업 후) 본인 비용으로 풀킷 구매 + 데모보드 구매 + `hardware/hardware_test.py`로 실물 검증
